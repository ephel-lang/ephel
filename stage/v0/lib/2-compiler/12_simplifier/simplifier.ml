...
let rec simplify_sequence =
  ...
  | DUP (i, m) :: DROP (j, n) :: l when j > i -> DROP (j - 1, n) :: DUP (i, m) :: l
  | DUP (i, m) :: DROP (j, n) :: l when j > 0 -> DROP (j - 1, n) :: DUP (i - 1, m) :: l
  | DIG (i, m) :: DROP (j, n) :: l when j > 0 ->
    if j > i then DROP (j - 1, n) :: DIG (i, m) :: l else DROP (j - 1, n) :: DIG (i - 1, m) :: l
