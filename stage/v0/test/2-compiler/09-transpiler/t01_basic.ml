...
let compile_06 () =
  let result = compile (App (App (Abs ([ "x"; "y" ], Var "y"), Int 1), Int 2))
  and expected =
    [
      LAMBDA ([ "x"; "y" ], [ DUP (0, "y"); DROP (1, "y"); DROP (1, "x") ])
    ; PUSH (INT 1)
    ; EXEC
    ; PUSH (INT 2)
    ; EXEC
...
let compile_07 () =
  (* PARTIAL APPLICATION *)
  let result = compile (App (App (Abs ([ "x"; "y" ], Var "x"), Int 1), Int 2))
  and expected = [ PUSH (INT 1) ] in
  Alcotest.(check (result string string))
...
*)
let compile_09 () =
  let result = compile (Abs ([ "f"; "x" ], App (Var "f", Var "x")))
  and expected = [ LAMBDA ([ "f"; "x" ], [ DIG (1, "f"); EXEC ]) ] in
  Alcotest.(check (result string string))
    "compile (fun f x -> f x)"
    (return expected <&> to_string)
    (result <&> to_string)
...
let compile_10 () =
  ...
  and expected =
    [
      LAMBDA
        ([ "f" ], [ PUSH (INT 1); DUP (1, "f"); DUP (1, "x"); EXEC; DROP (1, "x"); DROP (1, "f") ])
    ]
  in
  Alcotest.(check (result string string))
  ...
  , [
    ...
    ; test_case "compile O5" `Quick compile_05
      (* ; test_case "compile O6" `Quick compile_06
         ; test_case "compile O7" `Quick compile_07 ; test_case "compile O8" `Quick compile_08
         ; test_case "compile O9" `Quick compile_09 ; test_case "compile 10" `Quick compile_10 *)
    ] )
