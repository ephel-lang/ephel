...
let compile_09 () =
  ...
  and expected =
    [
      LAMBDA
        ( [ "y" ]
        , [ DUP (0, "y"); CASE ([ DROP (0, "x"); DROP (0, "y"); PUSH UNIT ], [ DROP (0, "x") ]) ] )
    ]
  in
  Alcotest.(check (result string string))
...
let compile_10 () =
  ...
  and expected =
    [
      LAMBDA
        ( [ "y" ]
        , [ DUP (0, "y"); CASE ([ DROP (0, "x"); DROP (0, "y"); PUSH UNIT ], [ DROP (1, "y") ]) ] )
    ]
  in
  Alcotest.(check (result string string))
...
let compile_11 () =
  let result =
    compile (Abs ([ "x" ], Case (Inl (Var "x"), Abs ([ "x" ], Var "x"), Abs ([ "x" ], Int 3))))
